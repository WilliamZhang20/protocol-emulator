...
// Instruction fetch/execute FSM plus decoded resource strobes. Shared engines
// stay outside; this module only decides when to launch and when to wait.
module vm_sequencer (
    input  wire       clk,
    input  wire       rst_n,
    input  wire       enable,
  ...
  assign execute_shift_clear = state == STATE_EXECUTE && opcode == 4'ha;
  ...
  assign bit_xfer_start = state == STATE_OPERAND_EXT_WAIT && !bit_xfer_busy;
